...
export type RevealParams = {
  ...
  /** Noise 0..0.5. */
  noise: number;
  /** Trail decay 0.8..0.995. */
  decay: number;
  /** Hover reveal strength 0..1 (caps the cursor trail so it isn't "burned"). */
  hoverReveal: number;
  /** Permanent center reveal strength 0..1. */
  baseReveal: number;
  /** Permanent center reveal radius 0..1. */
...
export class RevealRenderer {
  ...
  private trailU: Record<string, WebGLUniformLocation | null>;
  private compU: Record<string, WebGLUniformLocation | null>;
  private vao: WebGLVertexArrayObject;
  private pp: [Target | null, Target | null] = [null, null];
  private src = 0;

  private bgTex: WebGLTexture;
  private imageRes: [number, number] = [1200, 675];
  private imageAspect = 1200 / 675;

  private params: RevealParams;
  private dpr = 1;
  private cw = 0;
  private ch = 0;
  ...
  constructor(canvas: HTMLCanvasElement, params: RevealParams, imageSrc: string) {
    this.canvas = canvas;
    this.params = { ...params };
    ...
    if (!bgTex) throw new Error("Failed to create texture");
    this.bgTex = bgTex;
  ...
  private draw(now: number) {
    ...
    gl.uniform1f(
      this.compU.uBaseReveal ?? null,
      this.params.baseReveal * (1 - this.hoverAmt),
    );
  ...
  setParams(partial: Partial<RevealParams>) {
  ...
  setMouse(x: number, y: number, present: boolean) {
