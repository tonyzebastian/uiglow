...
void main() {
  ...
  // Keep the large anchor dots sharp. The regular field can retain its soft
  // grain, but a wide feather on the anchors makes the logo silhouette look
  // blurred on a white ground.
  float softSprite = 1.0 - smoothstep(0.55, 1.0, d);
  float crispSprite = 1.0 - smoothstep(0.88, 1.0, d);
  float sprite = mix(softSprite, crispSprite, v_isAnchor);
